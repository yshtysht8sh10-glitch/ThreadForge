...
- ビルドとリリースはフロントエンド単位で進めます。リリーススクリプトにフロントIDを指定し、そのフロントと共通バックエンドを梱包します。
...
- 各フロントは独立したセマンティックバージョンを持ちます。`image-board` と `file-uploader` のバージョンは一致させる必要がありません。
- アプリのバージョンは各 `frontends/<frontend-id>/package.json` を基準にし、リリースZIP名にも含めます。
- Gitタグは `image-board-v0.9.7`、`file-uploader-v0.1.0` のようにフロントIDを付けます。
- ルートのバージョンは開発ワークスペース全体の管理用とし、単体アプリのリリース番号には使いません。
- 共通バックエンドは各アプリのリリースへ同梱します。APIに互換性のない変更が入る場合だけ、別途API互換バージョンを更新します。
