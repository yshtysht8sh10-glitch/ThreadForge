...
管理画面のフルバックアップは、対応ブラウザでは保存先ダイアログを開き、生成したZipを選択先へ直接保存します。配布Zipには大容量バックアップ復元用の `.user.ini` を同梱し、アップロード上限を1GBに設定します。レンタルサーバー側が `.user.ini` を上書き・無効化している場合は、サーバー管理画面で `upload_max_filesize` と `post_max_size` を調整してください。

復元には「フルバックアップをダウンロード」で作成したZipを指定します。素材カードから取得した `material-*-archive.zip` は素材本体であり、復元には使用できません。復元後はDB内の管理パスワードもバックアップ時点へ戻るため、そのパスワードで管理画面へ入り直します。

復元中は処理段階と経過時間を管理画面へ表示します。大容量バックアップでは、アップロード後のZip展開、DB差し替え、素材ファイル復元に数分かかる場合があります。完了またはエラーが表示されるまで画面を閉じないでください。
