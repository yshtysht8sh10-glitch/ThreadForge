...
管理画面の保守にあるフルバックアップ エクスポートボタンで、次を含む ZIP ファイルをダウンロードできます。
...
- アクセスカウンタ履歴
- 設定
- `images/` に格納された投稿画像とユーザーアイコン

ログインセッションは含めません。復元後は再ログインしてください。

フルバックアップZIPのインポートは完全復元です。バックアップ対象のDB行と画像を置き換えたうえで、バックアップ内の設定を復元します。過去バージョンのJSON形式バックアップもインポート互換として読み込めます。
