...
function MaterialForm(props: CommonProps & { mode: 'create' | 'edit'; item?: MaterialItem; initialPassword?: string }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const archiveFile = archive ?? archiveInputRef.current?.files?.[0] ?? null;
    const imageFile = activeImage ?? imageInputRef.current?.files?.[0] ?? null;
...
export async function exportFullBackup(password: string, notice: (message: string) => void, error: (message: string) => void) {
  ...
  try {
    ...
    const body = new FormData();
    body.set('frontend_id', frontendId);
...
export async function importBackup(
  ...
  const body = new FormData(); body.set('frontend_id', frontendId); body.set('action', 'importBackup'); body.set('admin_password', password); body.set('backup', file);
  try {
