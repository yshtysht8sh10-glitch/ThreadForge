// Synchronized from WebMUGEN src/parser/def/DefParser.ts at d5f4a2b.
// This local copy keeps the two deployable applications independent.
...
export function getCharacterDefFiles(document: DefDocument): CharacterDefFiles {
  ...
  for (const [key, value] of files.values.entries()) {
    ...
    if (paletteMatch) palettes.push({ slot: Number(paletteMatch[1]), file: value });
  }
